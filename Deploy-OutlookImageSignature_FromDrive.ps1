...
.DESCRIPTION
  Handles two packaging styles:
   ...
   B) Single ZIP that contains many per-user ZIPs (e.g., jbean.zip, bflanagan.zip), each with that user's images.
  The script will pick the correct per-user asset by matching:
     Preferred: -ImagePattern (UPN | SAM | DisplayName)
     Fallbacks: the other keys in order
...
.PARAMETER SignatureNa#requires -Version 5.1
<#
.SYNOPSIS
  Download a ZIP (or per-user ZIPs) from Google Drive, extract the proper JPG/PNG for the current user,
  and deploy it as the default Outlook signature (classic Outlook).
...
function Invoke-GDriveDownload {
  <#
    Downloads a Google Drive file by ID, handling the confirm token for large files.
    Uses Invoke-WebRequest with a persisted cookie session (PS 5.1 compatible).
  #>
  param(
    [Parameter(Mandatory=$true)][string]$FileId,
    [Parameter(Mandatory=$true)][string]$DestinationPath
...
function Get-FallbackKeyOrder {
  param([string]$Preferred)
  $all = @("UPN","SAM","DisplayName")
  @($Preferred) + ($all | Where-Object { $_ -ne $Preferred }) | Select-Object -Unique
}

function Try-FindInnerUserZip {
  <#
    In a folder extracted from the main ZIP, try to find a per-user ZIP that matches
    UPN/SAM/DisplayName. This enables packages like jbean.zip inside the main archive.
  #>
  param(
    [string]$Folder,
    [pscustomobject]$Ctx,
  ...
  foreach ($key in $order) {
    $name = $Ctx.$key
    if (-not $name) { continue }
    # candidates: exact.zip, lowercase.zip
    $cands = @(
      ($name + ".zip"),
      ($name.ToLower() + ".zip")
    )
    foreach ($z in $zips) {
      if ($cands -contains $z.Name) {
        return $z.FullName
      }
    }
  }
...
function Find-UserImageInFolder {
  param(
    [string]$Folder,
    [pscustomobject]$Ctx,
    [string]$PreferredPattern # UPN/SAM/DisplayName
  )
  $extensions = @(".jpg",".jpeg",".png")
  $order = Get-FallbackKeyOrder -Preferred $PreferredPattern
...
try {
  ...
  $ctx = Get-UserContext
  Write-Log "User context -> UPN='$($ctx.UPN)', SAM='$($ctx.SAM)', DisplayName='$($ctx.DisplayName)'"
  ...
  $srcImage = Find-UserImageInFolder -Folder $searchRoot -Ctx $ctx -PreferredPattern $ImagePattern
  if (-not $srcImage) {
    throw "Could not locate an image for user using UPN/SAM/DisplayName (.jpg/.jpeg/.png)."
  }
  Write-Log "Matched user image: $srcImage"
